// js/training-runner.js - 训练执行引擎（含 lastUsed 记录 & 下一个动作预览 & 提前显示动作名）
(function() {
  'use strict';
    ...
    const engineOptions = {
      callbacks: {
        ...
        onGroupStart: (data) => {
          ...
          // ★ 新增：提前更新左上角显示当前动作名和组信息
          if (data.project) {
            const projectName = data.project.name || '未命名';
            document.getElementById('training-project-name').textContent = projectName;
            document.getElementById('training-phase-label').textContent = '准备';
            const groupInfo = `第 ${data.group}/${data.totalGroups} 组 · 第 ${data.rep || 1}/${data.totalReps || 1} 次`;
            document.getElementById('training-note').textContent = groupInfo;

            // 提前加载当前动作的配图（如果已有）
            const img = document.getElementById('training-image-img');
            if (data.project.imageData && data.project.imageData.startsWith('data:image')) {
              img.src = data.project.imageData;
        ...
        onRestStart: (data) => {
          console.log(`📍 [onRestStart] 回调触发, type: ${data.type}, nextStep:`, data.nextStep);
          ...
          if (data.type === 'action' && nextStep && nextStep.kind === 'project') {
            const nextProject = projects.find(p => p.id === nextStep.projectId);
            if (nextProject) {
              document.getElementById('training-note').textContent = `⏩ 下一个动作：${nextProject.name}`;
              const img = document.getElementById('training-image-img');
              if (nextProject.imageData && nextProject.imageData.startsWith('data:image')) {
                img.src = nextProject.imageData;
                img.style.display = 'block';
              } else {
                img.style.display = 'none';
              }
            } else {
              document.getElementById('training-note').textContent = '⏩ 下一个动作：未知';
            }
          } else {
        ...
        onComplete: (data) => {
          console.log(`📍 [onComplete] 回调触发，记录最后使用时间`);

          (async () => {
            try {
              const routine = await DB.getRoutine(currentRoutineId);
              if (routine) {
                routine.lastUsed = new Date().toISOString();
                await DB.saveRoutine(routine);
                console.log(`✅ 已更新训练组 "${routine.name}" 的最后使用时间`);
              }
            } catch (e) {
              console.warn('⚠️ 更新 lastUsed 失败:', e);
            }
          })();
  ...
  console.log('✅ TrainingRunner 已加载（含 lastUsed 记录 & 下一个动作预览 & 提前显示动作名）');
})();